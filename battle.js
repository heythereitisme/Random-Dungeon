...
let special = async (dv, enemy) => {
  if (enemy === "Goblin Warrior") {
    ...
    console.log(
      "\x1b[32mYou are cut with a poisoned blade! You are poisoned!\x1b[0m"
    );
    playerPoison = true;
    return await damage(dv);
  } else if (enemy === "Dual Blader") {
    console.log(
      "\x1b[31mWielding both blades, the Dual Blader strikes twice!\x1b[0m"
    );
    ...
    console.log(
      "\x1b[31mThe cannoneer fires a great blast, but he has to reload!\x1b[0m"
    );
    dv = dv * 3;
    reload = 2;
    return await damage(dv);
    ...
    console.log(
      "\x1b[31mThe knight performs a defensive charge, activating his guard!\x1b[0m"
    );
    ...
    tangle++;
    console.log(
      "\x1b[31mThe mummy tangles you in it's bandages, it gets tighter after every use!\x1b[0m"
    );
    dv = dv * tangle;
    return await damage(dv);
  } else if (enemy === "Quickblader") {
    console.log(
      "\x1b[31mThe quickblader uses his speed to gain an extra turn!\x1b[0m"
    );
    speed = true;
    return await damage(dv);
  ...
  battleloop: while (true) {
    ...
    if (playerPhase === "1") {
      ...
      if ((await checkItem("Dagger")) === "true") {
        if (dCoin() === 0) {
          console.log(
            `\x1b[35mYour dagger slices true. Dealt ${attackValue} damage\x1b[0m`
          );
          await slowDown(1000);
          await addScore(attackValue);
          enemyHP = enemyHP - attackValue;
      ...
      if (clone === true) {
        ...
        console.log(
          `\x1b[35mYour clone attacks dealing ${cloneAttack} damage!\x1b[0m`
        );
        await slowDown(1000);
          ...
          if (skillMenu === "1") {
            ...
            if ((await manaCheck()) >= 25) {
              ...
              if ((await checkItem("Strength Bracelet")) === "true") {
                console.log(
                  "\x1b[35mYour Strength Bracelet improves your swing!\x1b[0m"
                );
            ...
            if ((await checkItem("Mega Hammer")) === "true") {
              ...
              console.log("You do not have the Mega Hammer");
            }
          } else if (skillMenu === "4") {
            // summon clone
            if ((await checkItem("Summoning Scroll")) === "true") {
              if (clone === false) {
                ...
                console.log("Clone already active.");
              }
            } else {
              console.log("You do not have the Summoning Scroll");
            }
      ...
      while (true) {
        let itemInput = rl.question(
          `Do you want to use HP Potions - ${hpPots} owned (1), MP Potions - ${mpPots} owned (2), Smoke Bomb - ${smokeBombs} owned (3) or return to previous menu (4)?\n`
        );
        if (itemInput === "1") {
